...
while ( <CONFIGURE_DEFAULTS> )
{
  ...
  if ( $latchon == 1 )
  {
	...
	$_ =~ s:CONFIGURE_NETCDF_LIB_PATH:-L$sw_netcdf_path/lib -lnetcdf: ;
	 }
    else                   
...
#printf "Settings are written to the file configure.arwp here in the top-level\n" ;
...
printf "Configuration successful. To build the ARWpost, type: compile \n" ;
printf "------------------------------------------------------------------------\n" ;
